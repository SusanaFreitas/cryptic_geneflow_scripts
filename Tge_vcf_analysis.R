...
#################################### STACKS ############################################
########################################################################################
### use for all analyses STACKS output files

#### ADEGENET

### make a PCA plot

Tgen <- read.genepop("populations.snps.gen", quiet = TRUE)
# Allele presence absence data are extracted and NAs replaced usingtab:
X <- tab(Tgen, NA.method="mean")
## make PCA
pca1 <- dudi.pca(X,scannf=FALSE,scale=FALSE)
temp <- as.integer(pop(Tgen))
myCol <- transp(c("blue","red"),.7)[temp]
myPch <- c(15,17)[temp]
## basic plot
pdf(file="PCA_Tge.pdf")
pdf(file="PCA_Tge_labels.pdf")
plot(pca1$li, col=myCol, cex=3, pch=myPch, xlim =c(-400, 400), ylim=c(-100, 200))
dev.off()
...
# Tgenew <- nei.dist(Tgen, warning = TRUE)
...
tre <- nj(D)
par(xpd=TRUE)
pdf(file="Tge_dist.pdf")
pdf(file="Tge_dist_edgelab.pdf")
...
s.label(pco1$li*1.1, label=0, ppoints.pch=1)
...
## read vcf file
vcf3 <- read.vcfR("populations.snps.vcf") #read in all data
...
# gather fst
fst <- myData %>% select(fst, smooth_amova_fst) %>% gather(key = "stat", value = "fst")
fst$stat <- sub("_fst", "", fst$stat)

# make a plot
pdf(file="Tge_FST_betweenloci.pdf")
...
ggsave("./Tge_fst_plot.pdf", a, width = 6, height = 8)
...
LDscaf <- read.delim(file = "Tge.scaf.snp.geno.ld", header=TRUE)
...
LDscaf$new1 <- paste(LDscaf$chr, "-", LDscaf$POS1, sep="") # new1 corresponds to the POS1
LDscaf$new2 <- paste(LDscaf$chr, "-", LDscaf$POS2, sep="") # new2 corresponds to the POS2
head(LDscaf)

LDscaf$new1 <- as.character(LDscaf$new1)
ggplot(LDscaf, aes(R.2, y = new1)) + 
    geom_point(aes(y = y1, col = "y1")) + 
    geom_point(aes(y = y2, col = "y2"))

## alternate between 2 colours
LDscaf$col = ifelse(as.numeric(LDscaf$chr) %% 2 == 0, 0, 1)
## from here: https://digibio.blogspot.com/2018/05/ggplot-and-alternate-shading-with.html
pdf(file="Tge_LD_alongscaffs.pdf", width = 10)
ggplot(LDscaf, aes(x = new1, y = R.2)) + 
...
pdf(file="Tge_r2intra_viol.pdf")
ggplot(LDscaf, aes(factor(sps), R.2)) + 
	geom_violin()
dev.off()

## plot only R2 < 1
LD2 <- LDscaf[ which(LDscaf$R.2<1), ]
pdf(file="Tge_r2intra_wout1_viol.pdf")
ggplot(LD2, aes(factor(sps), R.2)) + 
...
## read vcf file
vcf <- read.vcfR("Tge.fb.vcf") #read in all data
vcf2 <- read.vcfR("corrected_output_allxuniq.vcf") #read in all data
head(vcf) #check the vcf object
vcf@fix[1:10,1:5] #check
