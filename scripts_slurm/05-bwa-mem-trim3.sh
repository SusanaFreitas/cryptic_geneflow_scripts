...
# outputs
#SBATCH --output=TgeTRIM2.out
#SBATCH --error=TgeTRIM2.err
...
for i in $(cat inds); do
        bwa mem -t 4 -M -R "@RG\tID:$i\tSM:$i" 5_Tge_b3v08.fasta $i-trim3.fq.gz > $i-map3.sam
        echo $i
done
